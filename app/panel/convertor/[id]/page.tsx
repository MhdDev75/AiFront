        ...
        <section className="flex flex-col gap-3 pb-3">
            ...
            <div className='card bg-base-300 flex flex-col gap-3  shadow-xl p-3'>
                <div className='flex flex-row flex-nowrap gap-1 items-center w-full'>
                    <select value={selectedValue} onChange={(e) => setSelectedValue(e.target.value)} className="select select-bordered border-primary w-full max-w-xs">
                        <option disabled value="1">واحد</option>
                        {cmb.map((item) => (
                            <option key={item} value={item}>{item}</option>
                        ))}
                    </select>
                    <input type="number" onChange={(e) => setInput(Number(e.target.value))} inputMode='decimal' placeholder="مقدار را وارد نمایید" className="input input-bordered border-primary max-w-xs" />

                </div>
                <button onClick={() => handelClick()} className='btn btn-primary' disabled={loading} >
                    ...
                    {list.map((item) => (
                        <label key={item.id} className="form-control w-full">
                            ...
                            <label className="input input-bordered flex items-center gap-2 border-secondary">
                                ...
                                <input readOnly type="text" value={item.value.toLocaleString()} className="grow" />
                                <Copy color='purple' onClick={() => handelCopy(item.value.toString())} />
                            </label>
