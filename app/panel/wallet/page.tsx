    ...
    <div className="flex flex-col h-full gap-4">
      <div className="card bg-base-300 shadow rounded-3xl p-2 overflow-hidden">
        <div className="card-body p-2 text-center">
          ...
          <div className="flex flex-col gap-2">
            ...
            <span className="card-title text-3xl font-bold justify-center">
              {balanceServer
                ? Number(balanceServer.amount).toLocaleString() + " "
                : 0}{" "}
            </span>
          </div>
      ...
      <div className="flex flex-row justify-between px-3">
        <div className=" flex flex-col items-center gap-1">
          <button
            onClick={() => router.push("/panel/wallet/receipt")}
            className="btn btn-sm rounded-full h-12 w-12  btn-primary animate-pulse p-0"
          >
            <Banknote size={30} />
          </button>
          <span className="font-bold text-sm ">{t("wallet.TopUp")}</span>
        </div>
        <div className=" flex flex-col items-center gap-1">
          <button
            onClick={() => router.push("/panel/wallet/history")}
            className="btn btn-warning rounded-full h-12 w-12  p-0"
          >
            <History size={25} />
          </button>
          <span className="font-bold text-sm">{t("wallet.history")}</span>
        </div>
        <div className=" flex flex-col items-center gap-1">
          <button
            disabled
            className="btn btn-success rounded-full h-12 w-12  p-0"
          >
            <ArrowRightLeft size={25} />
          </button>
          <span className="font-bold text-sm">{t("wallet.Transfer")}</span>
        </div>
        <div className=" flex flex-col items-center gap-1">
          <button
            disabled
            className="btn btn-error rounded-full h-12 w-12  p-0"
          >
            <BackpackIcon size={25} />
          </button>
          <span className="font-bold text-sm">{t("wallet.Withdraw")}</span>
        </div>
      </div>
      <div className="flex flex-col h-[calc(100%-14rem)] gap-3">
        ...
        <div className="flex overflow-y-scroll flex-col gap-2">
          {transactionServer &&
            transactionServer.map((item, index) => (
              <InlineBoxComponent
                key={index}
